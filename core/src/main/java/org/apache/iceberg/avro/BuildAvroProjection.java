...
class BuildAvroProjection extends AvroCustomOrderSchemaVisitor<Schema, Schema.Field> {
  ...
  public Schema record(Schema record, List<String> names, Iterable<Schema.Field> schemaIterable) {
    ...
    for (int i = 0; i < expectedFields.size(); i += 1) {
      ...
      String fieldName = AvroSchemaUtil.makeCompatibleName(expectedField.name());
      ...
      if (avroField != null) {
        ...
        Preconditions.checkArgument(
            expectedField.isOptional()
                || expectedField.fieldId() == MetadataColumns.ROW_POSITION.fieldId()
                || expectedField.hasDefaultValue(),
            "Missing required field that has no default value: expectedField: %s, avroField: null, record: %s",
            expectedField,
            record);

        // Create a field that will be defaulted to the expectedField's default value. If no default
        // value,
        ...
        newField.addProp(AvroSchemaUtil.FIELD_ID_PROP, expectedField.fieldId());
        if (!expectedField.name().equals(fieldName)) {
          newField.addProp(AvroSchemaUtil.ICEBERG_FIELD_NAME_PROP, expectedField.name());
        }
        updatedFields.add(newField);
        hasChange = true;
      }
